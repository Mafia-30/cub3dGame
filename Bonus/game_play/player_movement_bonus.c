...
static int collision(double x, double y, t_map *map)
{
    ...
    while (i < 0.2)
    {
        ...
        while (j < 0.2)
        {
            if (map->map[(int)(y + i)][(int)(x + j)] == '1' || map->map[(int)(y + i)][(int)(x + j)] == 'D')
                return (1);
            j += 0.1;
        }
        i += 0.1;
...
static void	move_side(t_game *game, double side_step)
{
	...
	pseudo_x = 0;
	pseudo_y = 0;
	pseudo_x = game->map.player.x + cos(game->map.player.rotation_angle + M_PI_2) * side_step;
...
static void	move_front_back(t_game *game, double direct_step)
{
	...
	pseudo_x = 0;
	pseudo_y = 0;
	pseudo_x = game->map.player.x + cos(game->map.player.rotation_angle) * direct_step;
	pseudo_y = game->map.player.y + sin(game->map.player.rotation_angle) * direct_step;
...
void update_pos_dir(t_game *game)
{
    ...
    game->map.player.rotation_angle += game->map.player.rotation_speed * game->map.player.turn_dir;
    if (side_step)
