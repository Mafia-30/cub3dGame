...
static void draw_mini_player(t_game *game)
{
    ...
    draw_line(game->map.win_image, dx, dy, dx + 20 * cos(game->map.player.rotation_angle), 20 * sin(game->map.player.rotation_angle) + dy);
}
...
static void draw_maze2(t_game *game, int i, int j)
{
    ...
    sp.x = (game->map.player.x * game->tile_size) - (factor * game->map.mini_map_size / 2);
    sp.y = (game->map.player.y * game->tile_size) - (factor * game->map.mini_map_size  / 2);
