...
static void	get_first_intersection(t_game *game, t_point *intersect,
		bool *is_ray_left, double ray_angle)
{
	if (cos(ray_angle) < 0)
		*is_ray_left = true;
	else
	...
	if (!(*is_ray_left))
		intersect->x += 1;
	intersect->y = game->map.player.y + ((intersect->x - game->map.player.x)
			* tan(ray_angle));
}

static t_point	get_vert_intersect(t_game *game, double ray_angle,
		bool *foundVertHit)
{
	t_point	intersect;
	double	s_x;
	...
	*vert_intersect = get_vert_intersect(game, ray_angle, foundVertHit);
	if (*foundVertHit)
		distance = distanceBetweenPoints(game->map.player.x * game->tile_size,
				game->map.player.y * game->tile_size, vert_intersect->x
				* game->tile_size, vert_intersect->y * game->tile_size);
	if (*foundVertHit)
		return (distance);
	return (LONG_MAX);
}