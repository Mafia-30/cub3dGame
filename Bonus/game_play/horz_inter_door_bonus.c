...
static void	get_first_intersection(t_game *game, t_point *intersect, bool *is_ray_down, double ray_angle)
{
	if (sin(ray_angle) > 0)
		*is_ray_down = true;
	else
	...
	if (*is_ray_down)
		intersect->y += 1;
	intersect->x = game->map.player.x + ((intersect->y - game->map.player.y) / tan(ray_angle));
}
...
static t_point get_horz_intersect(t_game *game, double ray_angle, bool *foundHorzHit)
{
    t_point intersect;
	double s_y;
    double xstep;
    ...
    while (intersect.x > 0 && intersect.x < game->map.map_x
		&& intersect.y > 0 && intersect.y < game->map.map_y)
    {
		s_y = intersect.y;
		if (!is_ray_down)
			s_y -= 0.1;
		if (hasWallAt(game, intersect.x, s_y))
			break ;
        if (i == game->found_doors && hasDoorAt(game, intersect.x, s_y))
			return (*foundHorzHit = 1, intersect);
		else if (hasDoorAt(game, intersect.x, s_y))
			i++;
        intersect.x += xstep;
        intersect.y += ystep;
    }
