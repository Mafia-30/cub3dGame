...
static void	put_pixel_to_window(t_game *game, t_point startPoint, int i,
		t_point p_index)
{
	if (game->allow_door_drawing)
	{
		if (game->door_pixels[(int)((game->tile_size * p_index.y)
				+ p_index.x)] << 24)
			mlx_put_pixel(game->map.win_image, startPoint.x, startPoint.y + i,
				game->door_pixels[(int)((game->tile_size * p_index.y)
					+ p_index.x)]);
	}
	else if (game->intersection_type == 'r')
		mlx_put_pixel(game->map.win_image, startPoint.x, startPoint.y + i,
			game->ea_pixels[(int)((game->tile_size * p_index.y) + p_index.x)]);
	else if (game->intersection_type == 'l')
		mlx_put_pixel(game->map.win_image, startPoint.x, startPoint.y + i,
			game->we_pixels[(int)((game->tile_size * p_index.y) + p_index.x)]);
	else if (game->intersection_type == 't')
		mlx_put_pixel(game->map.win_image, startPoint.x, startPoint.y + i,
			game->no_pixels[(int)((game->tile_size * p_index.y) + p_index.x)]);
	else if (game->intersection_type == 'b')
		mlx_put_pixel(game->map.win_image, startPoint.x, startPoint.y + i,
			game->so_pixels[(int)((game->tile_size * p_index.y) + p_index.x)]);
}
...
void	draw_strip(t_point startPoint, double height, t_game *game)
{
	...
	while (i < height)
	{
		if (game->win_height > startPoint.y + i && startPoint.y + i >= 0
			&& game->win_width > startPoint.x && startPoint.x >= 0)
		{
