		...
		double *ray_distance)
{
	...
	*ray_distance = LONG_MAX;
	distances[0] = get_horz_door_distance(game, ray_angle, &foundHorzHit,
			&horz_intersect);
	distances[1] = get_vert_door_distance(game, ray_angle, &foundVertHit,
			&vert_intersect);
	...
	if (foundHorzHit || foundVertHit)
		*ray_distance = *ray_distance * cos(ray_angle
